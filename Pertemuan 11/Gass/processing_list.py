                    ...
                    for b in range(value):
                        if(t == 0):
                            if(not(((i-a) < 0) or ((j-b) < 0))):
                                ...
                                mask.append((0, 0, 0))
                        if(t == 1):
                            if(not(((i-a) < 0) or ((j+b) >=  (colOut-(int(value/2)))))):
                                if(not(b == 0)):
                                    mask.append(img_input.getpixel((i-a,j+b)))
                                else:
                                    mask.append((0, 0, 0))
                        if(t == 2):
                            if(not(((j-b) < 0) or ((i+a) >=  (rowOut-int(value/2))))):
                                if(not(a == 0)):
                                    mask.append(img_input.getpixel((i+a,j-b)))
                                else:
                                    mask.append((0, 0, 0))
                        if(t == 3):
                            if(not(((i+a) >=  (rowOut-int(value/2)) or ((j+b) >=  (colOut-int(value/2)))))):
                                if(not((a == 0) or (b == 0))):
                                    mask.append(img_input.getpixel((i+a,j+b)))
                                else:
                                    mask.append((0, 0, 0))
...
def Mean(img_input, coldepth, value):
    ...
    for i in range(1, rowOut-1):
        for j in range(1, colOut-1):
            ...
            r,g,b = 0,0,0
            for k in range((value*value)-1):
                r2, g2, b2 = mask[k]
                r2, g2, b2 = round (r2/(value*value)), round (g2/(value*value)), round (b2/(value*value))
...
def EdgeDetector(img_input, coldepth, value):
    if coldepth != 24:
        img_input = img_input.convert('RGB')
    ...
    for i in range(1, rowOut - 1):
        for j in range(1, colOut - 1):
            ...
            for x in range(8):
                r2, g2, b2 = mask[x]
                temp1.append(r2*mask_konvolusi[x])
                temp2.append(g2*mask_konvolusi[x])
                temp3.append(b2*mask_konvolusi[x])

            pixels[i, j] = (round(sum(temp1)), round(
                sum(temp2)), round(sum(temp3)))
            temp1 = []
            temp2 = []
            temp3 = []
...
def ImgGaussion(img_input, coldepth, value):
    if coldepth != 24:
        img_input = img_input.convert('RGB')
...
def ImgSharpen(img_input, coldepth, value):
    if coldepth != 24:
        img_input = img_input.convert('RGB')
...
def ImgErosi(img_input, coldepth, value):
    if coldepth != 24:
        img_input = img_input.convert('RGB')
    ...
    for i in range(rowOut):
        for j in range(colOut):
            for z in range(mask):
                if i + z - index < 0 or i + z - index > rowOut - 1:
                    for c in range(mask):
                        temp1.append(0)
                        temp2.append(0)
                        temp3.append(0)
                    ...
                    if j + z - index < 0 or j + index > colOut - 1:
                        temp1.append(0)
                        temp2.append(0)
                        temp3.append(0)
...
def ImgDilasi(img_input, coldepth, value):
    if coldepth != 24:
        img_input = img_input.convert('RGB')
...
def ImgOpening(img_input, coldepth, value):
    if coldepth != 24:
        img_input = img_input.convert('RGB')

    img_output_sementara = Image.new(
...
def ImgClosing(img_input, coldepth, value):
    if coldepth != 24:
        img_input = img_input.convert('RGB')
...
def ImgBlackTopHat(img_input, coldepth, value):
    if coldepth != 24:
        img_input = img_input.convert('RGB')
    ...
    for i in range(img_output_sementara.size[0]):
        for j in range(img_output_sementara.size[1]):
            img_output_sementara - img_input.getpixel((img_output.size[0]))
...
def ImgWhiteTopHat(img_input, coldepth, value):
    if coldepth != 24:
        img_input = img_input.convert('RGB')
